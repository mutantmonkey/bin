...
############### EDIT ME ############
deltadays="/usr/local/bin/deltadays.pl"
tmpdir="${HOME}/tmp"
mkdir -p "${tmpdir}"
...
podenc () {
   ...
   nice -n 19 ffmpeg -y $ratevar -i "${tmpfile1}" $ratevar $framesvar \
        ...
        -v 1 -vcodec libx264 -b ${video_bitrate} -bt 175k -refs 2 -flags +loop \
        -deblockalpha 0 -deblockbeta 0 \
        -partitions +parti4x4+partp8x8+partb8x8 \
        -me_method full -me_range 21 -subq 6 \
        -bf 0 -level 13 -g 300 -keyint_min 30 \
